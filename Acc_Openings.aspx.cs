...
public partial class AryavartBC_Acc_Opening : System.Web.UI.Page
{
      ...
      if (!string.IsNullOrEmpty(cspId))
      {
          ...
          if (dt.Rows.Count > 0)
        {
          ...
          if (i > 0)
          {
            ...
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Details Updated Successfully!');window.location ='Acc_Openings.aspx';", true);
          }
        }

        else
        {
          SqlCommand cmd = new SqlCommand("Insert into Acc_Openings(MemType,ReportDate, CspId, PMJDY, RD_NUMBER, FD_NUMBER, RD_AMOUNT, FD_AMOUNT) Values('ARYV',GETDATE(),'" + cspId + "','" + PMJDYInput.Text + "','" + RDInput.Text + "','" + FDInput.Text + "','" + RDAmountInput.Text + "','" + FDAmountInput.Text + "');", con);

          int i = cmd.ExecuteNonQuery();
          if (i > 0)
          {
            // Display an alert using ScriptManager
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Details Submitted Successfully!');window.location ='Acc_Openings.aspx';", true);
          }
        }
  ...
  public void mail(string to)
  {
    try
    {
      ...
      message.Subject = "Aaaryavart Account_Openings";
      ...
      var tables = new Action(() =>
      {
        ...
        tableHtml.Append("<thead><tr><th style='border: 1px solid black; width: 25%;'>Schemes</th><th style='border: 1px solid black; width: 25%;'>Daily Target</th><th style='border: 1px solid black; width: 25%;'>Number of Accounts Opened</th><th style='border: 1px solid black; width: 25%;'>Total Amount</th></tr></thead>");
        tableHtml.Append("<tbody>");
